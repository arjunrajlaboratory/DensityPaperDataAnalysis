...
genomicData <- read.delim('~/Dropbox/densitypaper/SingleCellSeqData/genomicDataFpkm.txt',header=T,stringsAsFactors=F)
genomicDataSub <- genomicData[,c('sampleID','type','sumCountsGenomic')]
genomicDataSub <- unique(genomicDataSub)
erccData <- read.delim('~/Dropbox/densitypaper/SingleCellSeqData/erccDataFpkm.txt',header=T,stringsAsFactors=F)
...
volumeData <- read.delim('~/Dropbox/densitypaper/SingleCellSeqData/fpkmRatio.txt',header=T,stringsAsFactors=F)
...
pdf(file='~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_ClassifyWeirdCells/gapdhWithFunkyCells.pdf',height=2,width=2.2)
gapdhWithFunkyCells
dev.off()

pdf(file='~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_ClassifyWeirdCells/erccGenomicCounts.pdf',height=2,width=2.2)
erccGenomicCounts
dev.off()

pdf(file='~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_ClassifyWeirdCells/weirdVsNormal.pdf',height=2,width=2.2)
weirdVsNormal
dev.off()
