...
crlDat <- read.delim('~/Dropbox/densitypaper/densitypaperdataanalysis/ExtractedData/CRL.txt',header=T,stringsAsFactors=F)
crlDat$type <- 'cycling'

ssDat <- read.delim('~/Dropbox/densitypaper/densitypaperdataanalysis/ExtractedData/SerumStarved.txt',header=T,stringsAsFactors=F)
ssDat$type <- 'quiescent'

senDat <- read.delim('~/Dropbox/densitypaper/densitypaperdataanalysis/ExtractedData/Senescent.txt',header=T,stringsAsFactors=F)
senDat <- subset(senDat,!(date %in% 140128))
senDat$type <- 'senescent'

a549Dat <- read.delim('~/Dropbox/densitypaper/densitypaperdataanalysis/ExtractedData/A549.txt',header=T,stringsAsFactors=F)
a549Dat$type <- 'a549'
...
pdf('~/Dropbox/densitypaper/densitypaperdataanalysis/SupplementaryFigures/DataDump/CRL_cycling_Conc.pdf',width=7.5,height=4)
ggplot(crlDat, 
...
pdf('~/Dropbox/densitypaper/densitypaperdataanalysis/SupplementaryFigures/DataDump/CRL_quiescent_Conc.pdf',width=7.5,height=4)
ggplot(ssDat, 
       aes(x=volume,y=cytoRNA/volume)) + 
  geom_point(size=1) + facet_wrap(~gene, scales = 'free_y') + expand_limits(x=0,y=0) + 
  theme_classic() +
...
pdf('~/Dropbox/densitypaper/densitypaperdataanalysis/SupplementaryFigures/DataDump/A549_cycling_Conc.pdf',width=7.5,height=4)
