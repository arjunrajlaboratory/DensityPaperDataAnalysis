...
concData <- read.delim('~/Dropbox/densitypaper/SingleCellSeqData/ercc_concentrations.txt',header=T,stringsAsFactors=F)
colnames(concData) <- c('ID','gene_id','subgroup','concMix1','concMix2','foldChange','log2')
erccReads <- read.delim('~/Dropbox/densitypaper/SingleCellSeqData/erccDataFpkm.txt',header=T,stringsAsFactors=F)
...
pdf(file='~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_CalibrateData/erccFpkmVsConcentration.pdf',height=3,width=3.3)
erccFpkmVsConcentration
dev.off()
