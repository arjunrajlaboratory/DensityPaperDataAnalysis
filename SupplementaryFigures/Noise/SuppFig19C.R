...
data <- read.delim('~/Dropbox/densitypaper/densitypaperdataanalysis/ExtractedData/SerumStarved.txt',header=T,stringsAsFactors=F)
...
highnoise <- subset(outMelt,gene %in% c('ACTA2','BCL3','FUCA1','MYC'))
lownoise <- subset(outMelt,!(gene %in% c('ACTA2','BCL3','FUCA1','MYC')))

pdf('~/Dropbox/densitypaper/densitypaperdataanalysis/SupplementaryFigures/Noise/Quies_highnoise.pdf',width=2.5,height=3)
ggplot(highnoise,aes(x=gene,y=value,fill=measure)) +
...
pdf('~/Dropbox/densitypaper/densitypaperdataanalysis/SupplementaryFigures/Noise/Quies_lownoise.pdf',width=5,height=3)
ggplot(lownoise,aes(x=gene,y=value,fill=measure)) +
  geom_bar(position='dodge',stat='identity',col='black',width=.9) +
  geom_errorbar(aes(ymin=lower,ymax=upper),position=dodge,width=.3) +
  theme_classic() +
  scale_fill_manual(values=cols) +
