...
crl <- read.delim('~/Dropbox/densitypaper/ExtractedData/CRL.txt',header=T,stringsAsFactors=F)
a549 <- read.delim('~/Dropbox/densitypaper/ExtractedData/A549.txt',header=T,stringsAsFactors=F)
quies <- read.delim('~/Dropbox/densitypaper/ExtractedData/SerumStarved.txt',header=T,stringsAsFactors=F)
...
pdf('~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_LowHi_SpecNon/Noise_CellTypes/cyc_vs_A549.pdf',width=2.2,height=2)
...
pdf('~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_LowHi_SpecNon/Noise_CellTypes/cyc_vs_A549_abund.pdf',width=2.2,height=2)
...
pdf('~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_LowHi_SpecNon/Noise_CellTypes/cyc_vs_quies.pdf',width=2.2,height=2)
ggplot(dat2, aes(x=est.crl,y=est.quies)) +
  geom_abline(slope=1,intercept=0,col='gray78') +
  geom_point(size=1,col=cols[3]) +
  ...
  expand_limits(x=c(.01,5),y=c(.01,5)) +
  scale_y_log10() + scale_x_log10() +
...
pdf('~/Dropbox/densitypaper/SupplementaryFigures/SingleCellSeq_LowHi_SpecNon/Noise_CellTypes/cyc_vs_quies_abund.pdf',width=2.2,height=2)
ggplot(dat2, aes(x=mean.crl,y=mean.quies,label=gene)) +
  geom_abline(slope=1,intercept=0,col='gray78') +
  geom_point(size=1,col=cols[3]) +
  scale_y_log10() + scale_x_log10() +
  geom_errorbarh(aes(xmin=meanlower.crl,xmax=meanupper.crl),height=.1,col=cols[3]) +
  geom_errorbar(aes(ymin=meanlower.quies,ymax=meanupper.quies),width=.1,col=cols[3]) +
  expand_limits(x=c(1,4000),y=c(1,4000)) +
  xlab('Mean (CRL)') + ylab('Mean (Quiescent)') +
  theme_classic() + 
  theme(axis.title=element_text(size="6"), axis.text=element_text(size='6'))
dev.off()
