function [] = extractalldata_si(filename);
...
fprintf(f,'exptType \t gene \t date \t type \t dataNum \t objNum \t');
fprintf(f,'numTxnSites \t numCyclin \t GAPDH \t numRNA \t intensity \n');
...
for i = 1:numel(mainDir)
    ...
    for k = 1:numel(contents)
        ...
        for j = 1:numel(objects)
            ...
            % RNA
            totalRNA = obj.channels.(exonChannel).numSpots;
            inNuc = numel(find(obj.channels.(exonChannel).inNucleus(dapiMask)==1));
            numRNA = totalRNA - inNuc;
            ...
            if length(Xs) == 0
                ...
                fprintf(f,'%s\t%s\t%s\t%s\t%d\t%d\t', cellType, gene, date, type, k, j);
                fprintf(f,'%d\t%d\t%f\t%d\t%f\n', numTxnSites, numCyclin, xVar, numRNA, intensity);
                continue;
            end
