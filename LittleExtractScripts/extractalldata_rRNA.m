function [] = extractalldata_rRNA(filename);
...
fprintf(f,'rrnaIntensityTotal \t rrnaIntensityAvg \t');
fprintf(f,'itsIntensityTotal \t itsIntensityAvg \t');
fprintf(f,'numCyclin \n');
...
for i = 1:numel(mainDir)
    ...
    for k = 1:numel(contents)
        ...
        for j = 1:numel(objects)
            ...
            %rRNA intensity
            [rrnaSum rrnaAvg] = getCellIntensity(obj,rrnaChannel,fillChannel);
            
            %ITS intensity
            [itsSum itsAvg] = getNucIntensity_rRNA(obj,itsChannel,fillChannel);
            ...
            fprintf(f,'%f\t%f\t', rrnaSum, rrnaAvg);
            fprintf(f,'%f\t%f\t', itsSum, itsAvg);
            fprintf(f,'%d\n', numCyclin);
        end
    end
