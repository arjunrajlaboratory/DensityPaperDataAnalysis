...
nucCrop = crop.*uint16(obj.channels.(nucColor).processor.mask);

sumIntensity = sum(nucCrop(:));
avgIntensity = sumIntensity/numel(find(obj.channels.(nucColor).processor.mask));