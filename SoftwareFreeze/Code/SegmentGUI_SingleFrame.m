...
function varargout = SegmentGUI(varargin)
    ...
    set(figH,'Visible','on');
    Hs = buildGUI(Hs,figH);
    Hs.fileNum = Hs.fileNums(1);  % start at first RNA file
    Hs = loadFileSet(Hs);
...
function Hs = makeAndShowOverlay(Hs)
    ...
    %-------------------------------------
    if isempty(Hs.imgH)
        Hs.imgH = imshow(Hs.TI,'Parent',Hs.imgAx);
    else
    ...
    if updateAllImgs
        ...
        if isempty(tF)
            ...
            Hs.TI = readmm(sprintf('%s%s%s%03d%s',Hs.dirPath,filesep,...
                            Hs.foundChannels{tF},Hs.fileNum,Hs.imgExts{tF}),1);
...
function Hs = segmentObject_Callback(hObject,eventdata)
    ...
    elseif ~isempty(hROI) 
        ...
        if sum(maskImg(:)) < 50  % user drew a tiny (NULL) ROI
            ...
            % nice looking ROI, create & append an |image_object|
            fnumStr = sprintf('%03d',Hs.fileNum);
            Hs.currObjs = [Hs.currObjs image_object(maskImg,fnumStr,Hs.dirPath)];
            Hs.currObjs(end) = Hs.currObjs(end).setDefaultProcessorsGaussian;
...
% data***.mat file and then return to previous set of image files.
    ...
    % Save the |image_object|s to data***.mat file
    objects = Hs.currObjs;
    save(sprintf('%s%sdata%03d.mat',filesep,Hs.dirPath,Hs.fileNum),'objects');
    clear objects;
    Hs.currObjs = [];
