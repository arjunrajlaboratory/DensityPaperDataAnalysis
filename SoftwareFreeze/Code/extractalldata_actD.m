function [] = extractalldata_actD(filename);
...
fprintf(f,'cellType \t gene \t date \t timePt \t dataNum \t objNum \t');
...
for i = 1:numel(mainDir)
    ...
    for k = 1:numel(contents)
        ...
        for j = 1:numel(objects)
            ...
            fprintf(f,'%s\t%s\t%s\t%s\t%d\t%d\t', cellType, gene, date, time, k, j);
            if isfield(obj.metadata,'volumeRealUnits')
